...
@mf_portfolio_bp.route('/<portfolio_id>/overview', methods=['GET'])
def get_portfolio_overview(user_email, portfolio_id):
    """
    Get lightweight portfolio overview (KPIs only, no positions list)
    GET /api/<user_email>/mf-portfolio/<portfolio_id>/overview
    """
    try:
        analysis = MFPortfolioService.get_portfolio_overview(user_email, portfolio_id)
        return jsonify(analysis), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
